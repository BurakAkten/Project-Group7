...
//const string ipAddress = "10.42.0.88";
        ...
        if(client.receive(&command, sizeof(Command)) != sizeof(Command)) {
            ...
            switch (command) {
                ...
                case Command::no_helmet: {
                    ...
                    pyrUp(frame, decompressedFrame);
                    cvtColor(decompressedFrame, decompressedFrame, CV_BGR2RGB);
                    ui->detectedImage->setPixmap(QPixmap::fromImage(QImage((unsigned char*) decompressedFrame.data, decompressedFrame.cols, decompressedFrame.rows, QImage::Format_RGB888)));
